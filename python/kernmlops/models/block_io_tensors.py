...
parser.add_argument("--input", type=str, default='', help="Input file name")
...
def explode_flags(df: pl.DataFrame) -> pl.DataFrame:
    df = df.with_columns([
            ...
            pl.when((pl.col("block_io_flags") & REQ_NOWAIT >
                     0)).then(True).otherwise(False).alias("nowait_flag"),]).drop("block_io_flags")
    return df
...
"""
if args.mode is not Mode.CAMP:
    train_df = pl.read_parquet("./rainsong_curated/block_io/*.parquet").filter(
        pl.col("device").is_in([
            271581184,
            271581185,
            271581186,
    ...
    test_df = pl.read_parquet("./rainsong_test_curated/block_io/*.parquet").filter(
        pl.col("device").is_in([
            271581184,
            271581185,
            271581186,
...
if args.mode is Mode.WHIZ:
    lag_cols = (['offset', 'size', 'queue_length_segment_ios', 'latency', 'throughput'] 
        + ["sync_flag", "metadata_flag", "fua_flag", "priority_flag", "nomerge_flag",
              "idle_flag", "readahead_flag", "background_flag", "nowait_flag"])
    import featurewiz
    train_df = pl.read_parquet(args.input).sort('ts_submit').drop('ts_submit')
    train_df_blah = explode_flags(train_df)
    ...
    train_df_blah = train_df_blah.with_columns(lagged_cols)
    train_df_blah = train_df_blah.drop(['latency', 'throughput'])

    raw_data = train_df_blah.to_pandas()
    outputs = featurewiz.featurewiz(dataname=raw_data, target="reject",
                                    corr_limit=1, verbose=2, test_data="", feature_engg="", category_encoders="")

    outputs[0].append("reject")
    to_select = outputs[0]

    train_df = train_df_blah.select(to_select)
    train_df.write_parquet(Path("train.fwhiz.parquet"))
...
if args.mode is Mode.INVE:
    train_df = convert_df(args.input)
...
if args.mode is Mode.CAMP:
    def filter_frame(df: pl.DataFrame) -> pl.DataFrame:
        df = df.filter(
            pl.col("device").is_in([
                271581184,
                271581185,
    ...
    def add_labels(df: pl.DataFrame) -> pl.DataFrame:
        df = df.with_columns([
            (pl.col('block_io_latency_us') < P85Prediction().threshold()).alias("label85"),
            (pl.col('block_io_latency_us') < P90Prediction().threshold()).alias("label90"),
            (pl.col('block_io_latency_us') < P95Prediction().threshold()).alias("label95"),
        ])
        return df

    start_path = Path("./rainsong_curated/block_io/")
    j = 0
    for i in start_path.glob("*.parquet"):
        print(i)
        train_df = add_labels(filter_frame(pl.read_parquet(i)))
        train_df.write_parquet(f"rainsong_labeled/{j}.parquet")
        j+=1
