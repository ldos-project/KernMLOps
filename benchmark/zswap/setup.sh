...
sudo perf stat \
    ...
    -o $USER_HOME/$output_dir/perf_results.txt \
    sudo cgexec \
    -g memory:benchmark_group \
    make -j$(nproc) \
    &>$USER_HOME/$output_dir/build_log.txt &&
    sudo grep -r . /sys/kernel/debug/zswap >$USER_HOME/$output_dir/final_zswap_stats.txt
