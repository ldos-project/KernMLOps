...
@triton.jit(do_not_specialize=list(range(12)))
def matmul_kernel(
...
a = torch.randn((BLOCK_SIZE_M, BLOCK_SIZE_K), device='cpu', dtype=DTYPE)
b = torch.randn((BLOCK_SIZE_K, BLOCK_SIZE_N), device='cpu', dtype=DTYPE)
c = None
torch_output = torch.matmul(a.to(torch.float32), b.to(torch.float32))
if PREPACKED:
    a, b, c = matmul_preprocess_input(a, b, c)
triton_output = matmul(a, b, c, 4, 4, 4)
