...
void execute_command(const char *command) {
    ...
    snprintf(full_command, sizeof(full_command), "stress-ng %s --quiet &", command);
    system(full_command);
}
...
int main(int argc, char *argv[]) {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        long elapsed = (loop_finish.tv_sec - loop_start.tv_sec) * 1000000000L + (loop_finish.tv_nsec - loop_start.tv_nsec);
